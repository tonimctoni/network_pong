...
func (b *Ball) move(p1pos, p2pos int16) int{
    if b.move_y(b.speed_y){
        b.speed_y=-b.speed_y
    }

    x_collision:=b.move_x(b.speed_x)

    if b.speed_x<0{
        if b.pos_x<=PLAYER_PADDING+PLAYER_WIDTH{
            if b.pos_y<=p1pos+PLAYER_HEIGHT && b.pos_y+BALL_SIZE>=p1pos{
                b.speed_x=-b.speed_x
                b.pos_x=PLAYER_PADDING+PLAYER_WIDTH
                return 0
            }
        }
    } else if b.speed_x>0 {
        if b.pos_x+BALL_SIZE>=WINDOW_WIDTH-(PLAYER_PADDING+PLAYER_WIDTH){
            if b.pos_y<=p2pos+PLAYER_HEIGHT && b.pos_y+BALL_SIZE>=p2pos{
                b.speed_x=-b.speed_x
                b.pos_x=WINDOW_WIDTH-(PLAYER_PADDING+PLAYER_WIDTH)-BALL_SIZE
                return 0
            }
        }
    } else {
        log.Fatalln("b.speed_x==0")
    }
